...
static constexpr float textPadding = 0.8f;
static constexpr float textSize = 1.0f;
static constexpr float textHeight = textSize * 8.5f;
static constexpr float categoryMargin = 0.5f;
static constexpr float paddingRight = 13.5f;
static constexpr float crossSize = textHeight / 1.5f;
static constexpr float crossWidth = 0.1f;
...
void ClickGui::renderTooltip(string* text) {
	...
	DrawUtils::drawRectangle(rectPos, MC_Color(235, 60, 203), 1.f, 0.7f);
		...
		for (auto& mod : moduleList) {
			...
			if (allowRender) {
				...
				if (!ourWindow->isInAnimation && !isDragging && rectPos.contains(&mousePos)) {
					...
					DrawUtils::fillRectangleA(vec4_t(rectPos.x + 1, rectPos.y + 1, rectPos.z - 1, rectPos.w - 1), mod->isEnabled() ? MC_Color(interfaceColor) : MC_Color(60, 60, 60, 90));
					...
					DrawUtils::fillRectangleA(vec4_t(rectPos.x, rectPos.y, rectPos.z, rectPos.w), MC_Color(0, 0, 0, 90));
					DrawUtils::fillRectangleA(vec4_t(rectPos.x + 1, rectPos.y + 1, rectPos.z - 1, rectPos.w - 1), mod->isEnabled() ? MC_Color(interfaceColor) : MC_Color(0, 0, 0, 60));
				}
			}
