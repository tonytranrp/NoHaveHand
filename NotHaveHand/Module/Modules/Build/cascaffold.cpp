...
cascaffold::cascaffold() : IModule(0x0, Category::COMBAT, "Place obsidian to crystal target!!!") {
	...
	registerBoolSetting("underanvil place", &under, under);
}
	...
	if (g_Data.getLocalPlayer()->getSelectedItemId() == 49 || aovcl) {
		...
		if (notarget) {
			if (!under) {
				...
				for (int x = targetPosi.x - 3; x <= targetPosi.x + 3; x++) {
					for (int z = targetPosi.z - 3; z <= targetPosi.z + 3; z++) {
						if (silentab) {
							slotab = supplies->selectedHotbarSlot;
							findObsi();
						}
			...
			if (under) {
				...
				for (int x = targetPosi.x - 3; x <= targetPosi.x + 3; x++) {
					for (int z = targetPosi.z - 3; z <= targetPosi.z + 3; z++) {
						...
						if (canpla(vec3_ti(x, (int)targetPosi.y - 2.6f, z))) {
							tryAutoTrap2(vec3_ti(x, (int)targetPosi.y - 2.6f, z));
						}
						if (silentab) {
							supplies->selectedHotbarSlot = slotab;
						}
