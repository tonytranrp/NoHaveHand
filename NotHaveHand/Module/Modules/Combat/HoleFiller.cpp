...
HoleFiller::HoleFiller() : IModule(0x0, Category::COMBAT, "Locate a good place to holecamp ;)") {
	...
	registerBoolSetting("No Target", &this->notarget, this->notarget);
	...
	if (!(g_Data.getLocalPlayer() == nullptr || !GameData::canUseMoveKeys())) {
		if (g_Data.isInGame()) {
			...
			if (notarget) {
				...
				vec3_t* targetpos = g_Data.getLocalPlayer()->getPos();
				for (int x = (int)targetpos->x - range; x < targetpos->x + range; x++) {
					for (int z = (int)targetpos->z - range; z < targetpos->z + range; z++) {
						for (int y = (int)targetpos->y - range; y < targetpos->y + range; y++) {
							vec4_t underline;
							vec3_t blockPos = vec3_t(x, y, z);
							vec3_t air = vec3_t(x, y + 1, z);
							vec3_t air2 = vec3_t(x, y + 2, z);
