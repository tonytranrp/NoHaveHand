...
void AutoTrap::onTick(C_GameMode* gm) {
	...
	if (!targetList15.empty()) {
		...
		vec3_t blockAbove5 = targetList15[0]->eyePos0;
		...
		blockAbove5.y -= targetList15[0]->height - 3;
		blockAbove5.z += targetList15[0]->height - 0;
		...
		if (place == 0) {
			...
			if (!tryAutoTrap(blockAbove3)) {
				blockAbove3.y -= 0;
				clientMessageF(": ) made by tony?");
			}


			if (!tryAutoTrap(blockAbove5)) {
				blockAbove5.y -= 0;
			}
			supplies->selectedHotbarSlot = slotab;
		}
