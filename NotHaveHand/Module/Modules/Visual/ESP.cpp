...
void findEntity111(C_Entity* currentEntity111, bool isRegularEntity) {
	...
	if (killauraMod->isMobEsp) {
		...
		if (!TargetUtil::isValidTarget(currentEntity111))
			return;
	}
...
void doRenderStuff(C_Entity* ent, bool isRegularEntitie) {
	...
	else if (TargetUtil::isValidTarget(ent)) {
		if (!(ent->damageTime > 1)) {
	...
	if (espMod->mode.getSelectedValue() == 2) DrawUtils::drawBetter2D(ent, (float)fmax(0.3f, 1 / (float)fmax(1, (*player->getPos()).dist(*ent->getPos()) * 3.f)));
