...
struct IModuleContainer {
	...
	IModuleContainer(shared_ptr<IModule> mod) {
		...
		if (moduleMgr->getModule<ArrayList>()->showkb) {
			if (keybind != 0x0) {
				char text[50];
				sprintf_s(text, 50, "%s%s", name, std::string(std::string(DARK_BLUE) + " [" + std::string(GREEN) + std::string(Utils::getKeybindName(keybind)) + std::string(YELLOW) + "]").c_str());
				moduleName = text;
				logF("made by tony?");
			}
		}
		...
		textWidth = DrawUtils::getTextWidth(&moduleName, 1.f) + 3.f;
	}
