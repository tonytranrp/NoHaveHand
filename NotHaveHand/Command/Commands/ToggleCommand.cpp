...
bool ToggleCommand::execute(std::vector<std::string>* args) {
	...
	if (mod->isHoldMode()) 
		clientMessageF("[NotHaveHand] %sModule cannot be toggled!", RED);
	else {
		mod->toggle();
		clientMessageF("[NotHaveHand] %s%s is now %s", GREEN, mod->getRawModuleName(), mod->isEnabled() ? "enabled" : "disabled");
	}
